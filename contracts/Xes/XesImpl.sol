...
contract XesImpl is XesStorage, XesAbstract, XesError, ExponentialNoError {
  ...
  event DistributionTokenUpdated(
    address oldDistAddress,
    address newDistAddress
  );
  ...
  function _initializeMarket(address cToken) internal {
    uint32 currentTimestamp = safe32(
      getTimestamp(),
      "timestamp timestamp exceeds 32 bits"
    );
  ...
  /**
   * @notice View then claimable comp accrued by holder in all markets and COMP speeds
  ...
  function calculatePendingBorrowerCompDelta(
    ...
    address borrower,
    Exp memory marketBorrowIndex,
    CompMarketState memory borrowState
  ) internal view returns (uint) {
    uint borrowIndex = borrowState.index;
    uint borrowerIndex = compBorrowerIndex[cToken][borrower];
  ...
  function _setDistributionToken(
    ...
    require(
      distributionToken != newDistributionToken &&
        newDistributionToken != address(0),
      "invalid distributionToken address"
    );
   ...
   */
  function getDistributionToken() public view virtual returns (address) {
